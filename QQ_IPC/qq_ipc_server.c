...
int main(void)
{
	...
	{               
        mkfifo(SERVER_PROT, 0664);						/*创建fifo管道*/
    }
    ...
    mylink_init(&head);                                 /*初始化链表(head == NULL)*/

    while (1) 
	{
        read(server_fd, &dbuf, sizeof(dbuf));           /*读取公共管道,分析数据包,处理数据*/
        switch (dbuf.protocal) 							/*协议号*/
		{
            case 1: login_qq(&dbuf, &head); break;    	/*1为登录*/ 
            case 2: transfer_qq(&dbuf, &head); break;   /*2为聊天*/ 
            case 4: logout_qq(&dbuf, &head); break;     /*4为退出*/
            default: err_qq(&dbuf);						/*其他则为不在线*/
        }
    }
